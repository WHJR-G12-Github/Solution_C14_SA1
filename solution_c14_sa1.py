...
class Bird:
    ...
    # Creating 'speed' and initializing it to 0
    speed=0
    def movedown(self):
        gravity=0.2
        # Changing 'speed' as 'self.speed'
        self.speed=self.speed+gravity
        # Changing 'speed' as 'self.speed'
        self.bird.y=self.bird.y+self.speed
    def moveup(self):
        # Changing 'speed' as 'self.speed' and assigning -10 to it
        self.speed=-10
    def display(self):
    ...
    for genome in generation:
        ...
        while True:  
          ...
          if bird1.bird.colliderect(pipe1.bpipe) or bird1.bird.colliderect(pipe1.tpipe) or bird1.bird.y>600 or bird1.bird.y<0:
              state="over"
          # Checking if 'state' is "over"    
          if state=="over":
               # Reassign "play" to 'state'
               state="play"
               # Increment 'birdcount' by 1
               birdcount+=1
